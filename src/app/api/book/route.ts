...
export async function POST(req) {
    try {
        ...
        return new Response(JSON.stringify({ message: "Book added successfully", newBook }), {
            headers: { 'Content-Type': 'application/json' }
        });
    } catch (error) {
        return new Response(JSON.stringify({ message: "Error adding book", error: error.message }), {
            status: 400,
            headers: { 'Content-Type': 'application/json' }
        });
    }
